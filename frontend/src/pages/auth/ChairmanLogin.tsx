...
export const ChairmanLogin: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const res = await api.post('/auth/chairman/login', { email, password });
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-center items-center p-6 relative">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl relative z-10">
        ...
        <h2 className="text-2xl font-bold text-white tracking-tight">Chairman Sign In</h2>
        <p className="text-xs text-slate-400 mt-1">Executive Control Portal Access</p>

        {successMessage && (
          <div className="mt-4 p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs flex items-center gap-2.5">
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          >
            {loading ? 'Authenticating...' : 'Sign In as Chairman'}
          </button>
        </form>
      </div>
    </div>
